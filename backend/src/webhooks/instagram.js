...
async function handleInstagramWebhook(req, res, io) {
  ...
  try {
    ...
    for (const entry of entries) {
      const igId = entry.id;
      const [businesses] = await pool.query('SELECT id FROM businesses WHERE ig_account_id=?', [igId]);
      ...
      for (const event of messaging) {
        ...
        let contactId;
        if (!contacts.length) {
          const [result] = await pool.query('INSERT INTO contacts (business_id, phone, name) VALUES (?, ?, ?)', [bizId, senderId, `IG_${senderId}`]);
