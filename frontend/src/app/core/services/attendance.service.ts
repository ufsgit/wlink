...
export class AttendanceService {
  ...
  addCheckIn(employeeName: string) {
    ...
    const existingRecord = records.find(r => r.employeeName === employeeName && r.date === today);
  ...
  addCheckOut(employeeName: string) {
    const records = this.getRecords();
    const now = new Date();
    const today = now.toISOString().split('T')[0];
    const timeString = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    
    const recordIndex = records.findIndex(r => r.employeeName === employeeName && r.date === today);
